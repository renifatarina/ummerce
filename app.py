...
def upload_image():
    ...
    if file and allowed_file(file.filename):
        ...
        # Get message
        query_string = "SELECT * FROM products WHERE pName LIKE %s ORDER BY id ASC"
        cur.execute(query_string, ('%' + 'shoes' + '%',))
        products = cur.fetchall()
        # Close Connection
        cur.close()
        flash('Showing result for: shoes', 'success')
        return render_template('render_result.html', products=products, filename=filename)
    else:
...
def list_product():
    image_list = LoadData().from_folder(['static/image/product'])
    image_list [:1000]
    Index(image_list).Start()
    SearchImage().get_similar_images(image_path=image_list[0],number_of_images=816)
    SearchImage().plot_similar_images(image_path = image_list[725])
