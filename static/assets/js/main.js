...
(function ($) {
	...
	var Porto = {
		...
		minipopup: {
			...
			action: '<a href="./carts" class="btn viewcart">View Cart</a><a href="./carts/checkout" class="btn btn-dark checkout">Checkout</a>',
			// option
		...
		productsCartAction: function () {
			...
			$('body').on('click', '.btn-add-cart.product-type-simple', function (e) {
				...
				if ($(this).closest('.product-default').length > 0) {
					...
					$productId = $(this).attr('href').trim().split('/').pop();
				} else {
					$product = $(this).closest('.product-row');
					$productId = $($product).find('.product-image-container').find('a').attr('href').trim().split('/').pop();
				}

				console.log($product.find('figure img'));
			...
			$('body').on('click', '.btn-add-cart.btn-add-cart-list-product', function (e) {
				e.preventDefault();
				var $product;
				var $productId;
				...
				// console.log($product.find('.product-title > a').attr('href'));
		...
		productsWishlistAction: function () {
			...
			$('body').on('click', '.btn-icon-wish:not(.added-wishlist)', function (e) {
				...
				var productId = $this.attr('href').trim().split('/').pop();

				$this.addClass('load-more-overlay loading');

				add2wishlist(productId, (result) => {
					$this.removeClass('load-more-overlay loading');
                    $this.addClass('added-wishlist ');
                    if ($this.find('span').text() !== '') {
                        $this.find('span').text('BROWSE WISHLIST');
                    }
                    $this.attr('href', '/user/wishlist/'+productId);
				...
				// $.post("./api/wishlist/" + productId, { "_token": "{{ csrf_token() }}" }, function (result) {
			...
			$('body').on('click', '.btn-icon-wish.added-wishlist', function (e) {
				...
				deleteWishlist(productId, (result) => {
					$this.removeClass('load-more-overlay loading');
                    $this.removeClass('added-wishlist ');
                    $this.attr('href', '/user/wishlist/'+productId);
                    $this.attr('title', 'Go to Wishlist');
					$('.wishlist-popup-msg').text('Product removed!');
                    $('.wishlist-popup').addClass('active');
                    setTimeout(function () {
                        $('.wishlist-popup').removeClass('active');
						$('.wishlist-popup-msg').text('Product added!');
                    }, 2000);
				}, (error) => {
					if (typeof error === 'object' && error !== null) {
						if (error.status === 401) {
	...
	Porto.initProductSingle = (function () {
		...
		var initVariation = function (self) {
			...
			self.$inputQty = self.$wrapper.find(".horizontal-quantity");
			...
			self.$inputQty.on("change", function () {
				const max = self.$inputQty.attr("max");
			...
			self.$btnCart.on('click', function (e) {
				e.preventDefault();
				const qty = self.$wrapper.find(".horizontal-quantity").val().trim();
				const stock = self.$inputQty.attr("max");
				const productId = $(this).attr('href').trim().split('/').pop();

				if (stock > 1 && qty > 0 && qty <= stock) {
					add2Cart(productId, qty, () => {
		...
		ProductSingle.prototype.variationMatch = function () {
			...
			if (stock < 1) {
				self.$btnCart.addClass('disabled');
				self.$btnCart.html('Stock Kosong');
			} else {
				self.$btnCart.removeClass('disabled');
			}
		}
